...
$username = $password = $confirm_password = $first_name = $last_name = $dob = $country = $gender = $contact = $role =  "";
$username_err = $password_err = $confirm_password_err = $first_name_err = $last_name_err = $dob_err = $country_err = $gender_err = $contact_err = $role_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    //validate role
    if(empty(trim($_POST["role"]) or !isset($_POST["role"]))){
    ...
    if(empty($role_err) && empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($first_name_err) && empty($last_name_err) && empty($dob_err) && empty($country_err) && empty($gender_err) && empty($contact_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO users (role, username, password, first_name, last_name, dob, country, gender, contact) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssss", $param_role, $param_username, $param_password, $param_first_name, $param_last_name, $param_dob, $param_country, $param_gender, $param_contact);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                echo "<script>alert('Registered succussful!'); window.location.href = '/SeventeenJuly/admin/staff_list/index.php?id=0';</script>";
            } else{
            ...
            <div id="content">
                ...
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                    ...
                    <span class="help-block"><?php echo $country_err; ?></span>
                     <!-- Role -->
                     <select class="inputbox <?php echo (!empty($role_err)) ? 'has-error' : ''; ?>" id="role" name="role" placeholder="Role" required>
